...
# --- UTILITIES / AGGRESSIV SIFFER-TVÄTT ---
...
if all([f_cw, f_lw, f_ly, f_inv]):
    ...
    # 2. Bearbeta Försäljning (Med SKU-tvätt för att undvika "New"-felet)
    def process_sales(df):
        ...
        # FIX: Strip och Upper för att säkerställa matchning mellan filer
    ...
    total_nmv_cw_eur = df_cw['NMV_EUR'].sum()
    nmv_cw_sek = total_nmv_cw_eur * ex_rate
    ...
    e1.metric("Current NMV", f"€{total_nmv_cw_eur:,.0f}")
    ...
    # ROW 2: SEK PERFORMANCE & TARGET GAPS
    ...
    # Hjälpvariabler för att skicka data till Analys-tabben
    m_comp_global = None
    camp_tab_global = None
    p1_active_mkt = None

    with tabs[0]: # HÄLSA (GROWTH MED ANDEL % OCH WoW %)
        ...
        for col, grp in zip([c1, c2], ['Brand', 'Article type']):
            ...
            h_m['Andel %'] = h_m['CW_EUR'] / total_nmv_cw_eur if total_nmv_cw_eur > 0 else 0
    ...
    st.info("Vänligen ladda upp Försäljning CW, LW, LY och Lagerrapport för att generera din board.")
